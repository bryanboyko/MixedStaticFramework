...
@interface MaxBrowser () <UIWebViewDelegate, UIActionSheetDelegate, UIAlertViewDelegate>
{
    MaxBrowserControlsView *browserControlsView;
...
@implementation MaxBrowser

#pragma mark - Init & dealloc

// designated initializer
- (id)initWithDelegate:(id<MaxBrowserDelegate>)delegate withFeatures:(NSArray *)p_sourceKitBrowserFeatures
{
    ...
    if (self) {
        ...
        if (p_sourceKitBrowserFeatures != nil && [p_sourceKitBrowserFeatures count] > 0)
        {
            for (NSString *feature in p_sourceKitBrowserFeatures)
            {
                ...
                [MaxCommonLogger debug:@"MaxBrowser" withMessage:[NSString stringWithFormat:@"Requesting SourceKitBrowser feature: %@", feature]];
            }
        }
    }
    return self;
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if(!browserWebView) {
        ...
        browserControlsView = [[MaxBrowserControlsView  alloc] initWithSourceKitBrowser:self];
...
- (void)loadRequest:(NSURLRequest *)request
{
    ...
    if (openSystemBrowserDirectly) {
        ...
        [MaxCommonLogger debug:@"MaxBrowser" withMessage:[NSString stringWithFormat:@"presenting browser from viewController: %@", currentViewController]];
...
{
    browserControlsView.MaxCommonBackButton.enabled = [webView canGoBack];
    browserControlsView.MaxForwardButton.enabled = [webView canGoForward];
...
- (void)dismiss
{
    [MaxCommonLogger debug:@"MaxBrowser" withMessage:@"Dismissing SourceKitBrowser"];
