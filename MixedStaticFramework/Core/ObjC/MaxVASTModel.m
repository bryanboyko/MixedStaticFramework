...
- (NSDictionary *)trackingEvents
{
    ...
    [MaxCommonLogger debug:@"VAST - Model" withMessage:[NSString stringWithFormat:@"returning event dictionary with %lu event(s)", (unsigned long)[eventDict count]]];
    for (NSString *event in [eventDict allKeys]) {
        NSArray *array = (NSArray *)[eventDict valueForKey:event];
        [MaxCommonLogger debug:@"VAST - Model" withMessage:[NSString stringWithFormat:@"%@ has %lu URL(s)", event, (unsigned long)[array count]]];
    }
    
    return eventDict;
}

- (MaxVASTUrlWithId *)clickThrough
{
...
- (NSArray *)mediaFiles;
{
    ...
    for (NSData *document in vastDocumentArray) {
        ...
        for (NSDictionary *result in results) {
            ...
            MaxVASTMediaFile *mediaFile = [[MaxVASTMediaFile alloc]
                                        initWithId:id_
                                        delivery:delivery
                                        type:type
                                        bitrate:bitrate
...
- (NSArray *)resultsForQuery:(NSString *)query
{
    ...
    for (NSData *document in vastDocumentArray) {
        ...
        for (NSDictionary *result in results) {
            ...
            MaxVASTUrlWithId *impression = [[MaxVASTUrlWithId alloc] initWithID:id_ url:[self urlWithCleanString:urlString]];
            [array addObject:impression];
        }
    }
    
    [MaxCommonLogger debug:@"VAST - Model" withMessage:[NSString stringWithFormat:@"returning %@ array with %lu element(s)", elementName, (unsigned long)[array count]]];
    return array;
}
