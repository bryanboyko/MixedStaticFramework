...
@class MaxVASTViewController;

@protocol MaxVASTViewControllerDelegate <NSObject>

@required

- (void)vastReady:(MaxVASTViewController *)vastVC;  // sent when the video is ready to play - required

@optional

- (void)vastError:(MaxVASTViewController *)vastVC error:(NSInteger)error;  // sent when any VASTError occurs - optional

// These optional callbacks are for basic presentation, dismissal, and calling video clickthrough url browser.
- (void)vastWillPresentFullScreen:(MaxVASTViewController *)vastVC;
- (void)vastDidDismissFullScreen:(MaxVASTViewController *)vastVC;
- (void)vastOpenBrowseWithUrl:(NSURL *)url vastVC:(MaxVASTViewController *)vastVC;
- (void)vastTrackingEvent:(NSString *)eventName;

@end

@interface MaxVASTViewController : UIViewController

@property (nonatomic, unsafe_unretained) id<MaxVASTViewControllerDelegate>delegate;
@property (nonatomic, strong) NSURL *clickThrough;
@property (nonatomic, strong) UIViewController *presenterViewController; // Change presenting view controller or add one if it was not injected in initializer

- (id)initWithDelegate:(id<MaxVASTViewControllerDelegate>)delegate withViewController:(UIViewController *)viewController;  // designated initializer for VASTViewController
