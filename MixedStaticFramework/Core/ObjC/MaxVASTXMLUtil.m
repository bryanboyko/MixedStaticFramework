...
void documentParserErrorCallback(void *ctx, const char *msg, ...)
{
    ...
    if ([errMsg length] > 0) {
        [MaxCommonLogger error:@"VAST - XML Util" withMessage:[NSString stringWithFormat:@"Document parser error: %@", errMsg]];
    }
    va_end(args);
}
...
void schemaParserErrorCallback(void *ctx, const char *msg, ...)
{
    ...
    if ([errMsg length] > 0) {
        [MaxCommonLogger error:@"VAST - XML Util" withMessage:[NSString stringWithFormat:@"Schema parser error: %@", errMsg]];
    }
    va_end(args);
}
...
void schemaParserWarningCallback(void *ctx, const char *msg, ...)
{
    ...
    NSString *errMsg = [[NSString stringWithCString:s encoding:NSUTF8StringEncoding] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    if ([errMsg length] > 0) {
        [MaxCommonLogger warning:@"VAST - XML Util" withMessage:[NSString stringWithFormat:@"Schema parser warning: %@", errMsg]];
    }
    va_end(args);
}
...
void schemaValidationErrorCallback(void *ctx, const char *msg, ...)
{
    ...
    NSString *errMsg = [[NSString stringWithCString:s encoding:NSUTF8StringEncoding] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    if ([errMsg length] > 0) {
        [MaxCommonLogger error:@"VAST - XML Util" withMessage:[NSString stringWithFormat:@"Schema validation error: %@", errMsg]];
    }
    va_end(args);
}
...
void schemaValidationWarningCallback(void *ctx, const char *msg, ...)
{
    ...
    NSString *errMsg = [[NSString stringWithCString:s encoding:NSUTF8StringEncoding] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    if ([errMsg length] > 0) {
        [MaxCommonLogger warning:@"VAST - XML Util" withMessage:[NSString stringWithFormat:@"Schema validation warning: %@", errMsg]];
    }
    va_end(args);
}
...
NSArray *performXPathQuery(xmlDocPtr doc, NSString *query)
{
    ...
    if (xpathCtx == NULL) {
        [MaxCommonLogger error:@"VAST - XML Util" withMessage:@"Unable to create XPath context."];
		return nil;
    }
    ...
    if (xpathObj == NULL) {
        [MaxCommonLogger error:@"VAST - XML Util" withMessage:@"Unable to evaluate XPath."];
		return nil;
    }
	
	xmlNodeSetPtr nodes = xpathObj->nodesetval;
	if (!nodes) {
        [MaxCommonLogger debug:@"VAST - XML Util" withMessage:@"Nodes was nil."];
		return nil;
	}
...
BOOL validateXMLDocSyntax(NSData *document)
{
    ...
    if (doc == NULL) {
        [MaxCommonLogger error:@"VAST - XML Util" withMessage:@"Unable to parse."];
		retval = NO;
    } else {
        xmlFreeDoc(doc);
    }
...
BOOL validateXMLDocAgainstSchema(NSData *document, NSData *schemaData)
{
    ...
    if (ret == 0) {
        [MaxCommonLogger error:@"VAST - XML Util" withMessage:@"document is valid"];
    } else if (ret > 0) {
        [MaxCommonLogger error:@"VAST - XML Util" withMessage:@"document is invalid"];
    } else {
        [MaxCommonLogger error:@"VAST - XML Util" withMessage:@"validation generated an internal error"];
    }
